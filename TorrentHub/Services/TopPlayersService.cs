...
public class TopPlayersService : ITopPlayersService
{
    ...
    private const string TopCoinsKey = "TopCoinsUsers";
    ...
    public async Task<List<UserPublicProfileDto>> GetTopPlayersAsync(TopPlayerType type)
    {
        string cacheKey = type switch
        {
            ...
            TopPlayerType.Coins => TopCoinsKey,
    ...
    public async Task RefreshTopPlayersCacheAsync()
    {
        ...
        // Top Coins
        var topCoins = await _context.Users
            .OrderByDescending(u => u.Coins)
            .Take(10)
            .Select(u => Mapper.ToUserPublicProfileDto(u))
            .ToListAsync();
        await _cache.SetStringAsync(TopCoinsKey, JsonSerializer.Serialize(topCoins), _cacheOptions);
        _logger.LogInformation("Refreshed Top Coins Users cache.");
